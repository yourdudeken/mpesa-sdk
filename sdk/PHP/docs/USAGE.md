...
This API is a version of the Business to Customer (B2C) API used to make payments from a Business to a Customer's business wallet, locally known as **Pochi la Biashara** (microSME).

Before using this method, ensure you have a B2C shortcode and the necessary permissions for B2Pochi.

To use this API, call the `b2pochi()` method on the `Mpesa` facade.

### Parameters

1. `phonenumber` - The recipient's phone number (MSISDN).
2. `amount` - Transaction amount.
3. `remarks` - Additional information (2–100 characters).
4. `occasion` (optional) - Additional optional information (1–100 characters).
5. `result_url` (optional) - Callback URL for the result.
6. `timeout_url` (optional) - Callback URL if the request times out.
...
$response = Mpesa::b2pochi(
    ...
    $amount      = 50,
    $remarks     = 'Payment to small vendor',
    $occasion    = 'Market Purchase'
);
