...
class MpesaConfig:
    ...
    b2c_consumer_key: Optional[str] = None
    b2c_consumer_secret: Optional[str] = None
    passkey: str
    shortcode: str
    till_number: Optional[str] = None
    initiator_name: str
    initiator_password: str
    b2c_shortcode: Optional[str] = None
...
class Mpesa:
    ...
    def _get_config(self, key: str, default=None):
        ...
        value = self.config
        for k in keys:
            value = getattr(value, k, None) if not isinstance(value, dict) else value.get(k)
            if value is None:
                return default
        return value
    
    def _resolve_callback_url(self, param_url: Optional[str], config_key: str) -> str:
        config_url = self._get_config(f'callbacks.{config_key}')
        if param_url:
            return param_url
        if config_url:
            return config_url
    ...
    def _generate_security_credential(self) -> str:
        ...
        cert_path = 'certificates/SandboxCertificate.cer' if self._get_config('environment') == 'sandbox' else 'certificates/ProductionCertificate.cer'
        ...
        encrypted = public_key.encrypt(password, serialization_padding.PKCS1v15())
